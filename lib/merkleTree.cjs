 ...
 * @returns 'left' | 'right'
 */
var getLeafSide = function (hash, merkleTree) {
    var hashIndex = merkleTree[0].findIndex(function (h) { return h === hash; });
    if (hashIndex === -1) {
...
function ensureHashesEven(hashes) {
    return hashes.length % 2 === 0
        ? __spreadArray([], hashes, true) : __spreadArray(__spreadArray([], hashes, true), [hashes[hashes.length - 1]], false);
}
/**
...
function generateMerkleRoot(hashes) {
    ...
    var evenHashes = ensureHashesEven(hashes);
    var combinedHashes = [];
    for (var i = 0; i < evenHashes.length; i += 2) {
        var hashPair = evenHashes[i] + evenHashes[i + 1];
        var hash = sha256(hashPair);
...
function generateMerkleTree(hashes) {
    ...
    var tree = [hashes];
    var generate = function (hashes, tree) {
        if (hashes.length === 1)
            return hashes;
        var evenHashes = ensureHashesEven(hashes);
        var combinedHashes = [];
        for (var i = 0; i < evenHashes.length; i += 2) {
            var concatHashes = evenHashes[i] + evenHashes[i + 1];
            var hash = sha256(concatHashes);
            combinedHashes.push(hash);
        }
...
var merkleTree = generateMerkleTree(HASH_VALUE);
